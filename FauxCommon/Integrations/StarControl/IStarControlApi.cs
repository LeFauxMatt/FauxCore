...
public interface IStarControlApi
{
    ...
    /// <param name="id">
    ///     Unique (per mod) ID for the page. Registering a page with a previously-used ID will overwrite
    ///     the previous page.
    /// </param>
    ...
    /// <remarks>
    ///     <para>
    ///         Items registered this way are not immediately associated with a menu page, and do not appear in an enforced
    ///         order;
...
public interface IRadialMenuPageFactory
{
    ...
    /// </remarks>
    /// <param name="who">
    ///     The player for whom the page will be displayed. Callers should use this whenever possible
    ///     instead of <see cref="Game1.player" /> in case of co-op play.
    /// </param>
...
public interface IRadialMenuPage
{
    ...
    bool IsEmpty() => !this.Items.Any(item => item is not null);
}
...
public interface IRadialMenuItem
{
    ...
    /// </summary>
    /// <param name="who">
    ///     The player who activated the item; generally,
    ///     <see cref="Game1.player" />.
    /// </param>
    /// <param name="delayedActions">
    ///     The types of actions which should result in a
    ///     <see cref="ItemActivationResult.Delayed" /> outcome and the actual action being
    ///     skipped.
    /// </param>
    /// <param name="activationType">
    ///     The type of activation requested, determined by which button
    ///     the player pressed and in what context.
    /// </param>
    ...
    ///     the player pressed and in what context.
    /// </param>
    /// <param name="defaultSound">
    ///     The default sound that would play if not overridden; implementers
    ///     should return this value if not changing the default.
    /// </param>
    /// <returns>
    ///     The name of the sound cue to play on activation, or <c>null</c> or an empty string
    ///     to play no sound.
    /// </returns>
